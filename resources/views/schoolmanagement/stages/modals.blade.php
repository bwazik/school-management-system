        ...
        <div class="card-header card-header-bazoka flex-column flex-md-row">
            <div class="dt-action-buttons pt-3 pt-md-0">
                <div class="dt-buttons btn-group flex-wrap">
                    <button class="btn btn-secondary btn-primary waves-effect waves-light me-2" tabindex="0" aria-controls="add-stage-modal" type="button" id="add-stage-button" data-bs-toggle="offcanvas" data-bs-target="#add-stage-modal">
                        <span>
                            <i class="ti ti-plus me-sm-1"></i>
                            <span class="d-none d-sm-inline-block">{{ trans('schoolmanagement/stages.addStage') }}</span>
                        </span>
                    </button>
                    <button class="btn btn-secondary btn-primary waves-effect waves-light me-2" tabindex="0" aria-controls="datatable" type="button" id="delete-selected-btn">
                        <span>
                            ...
                            <span class="d-none d-sm-inline-block">{{ trans('schoolmanagement/stages.deletSelected') }}</span>
                        </span>
                    </button>
                    <div class="btn-group">
                        <button class="btn btn-secondary buttons-collection dropdown-toggle btn-label-primary waves-effect waves-light" data-bs-toggle="dropdown" tabindex="0" aria-controls="datatable" type="button" aria-haspopup="dialog" aria-expanded="false">
                            <span><i class="ti ti-file-export me-sm-1"></i>
                                <span class="d-none d-sm-inline-block">{{ trans('schoolmanagement/stages.export') }}</span>
        ...
        <table id="datatable" class="datatables-basic table" style="white-space: nowrap;">
            <thead>
                <tr>
                    ...
                    <th>#</th>
                    <th>{{ trans('schoolmanagement/stages.name') }}</th>
                    <th>{{ trans('schoolmanagement/stages.createdDate') }}</th>
                    <th>{{ trans('schoolmanagement/stages.actions') }}</th>
                </tr>
            </thead>
        </table>
    </div>
...
<!-- Add Stage -->
<div class="offcanvas offcanvas-end" tabindex="-1" id="add-stage-modal" aria-labelledby="add-stage-modalLabel">
    <div class="offcanvas-header border-bottom">
        <h5 class="offcanvas-title">{{ trans('schoolmanagement/stages.addStage') }}</h5>
        <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body mx-0 flex-grow-0 pt-0 h-100">
        <form class="pt-0" id="add-form" action="{{ route('addStage') }}" method="POST">
            @csrf
            <div class="mb-3 mt-3">
                <label class="form-label" for="name_ar">{{ trans('schoolmanagement/stages.name_ar') }}</label>
            ...
            <div class="mb-3">
                <label class="form-label" for="name_en">{{ trans('schoolmanagement/stages.name_en') }}</label>
                <input type="text" id="name_en" class="form-control" name="name_en"
                    placeholder="ex : Primary Stage" />
                <span class="invalid-feedback" id="name_en_add_error" role="alert">
                    <strong></strong>
                </span>
            </div>
            <button type="submit" class="btn btn-success data-submit me-sm-3 me-1">{{ trans('schoolmanagement/stages.submit') }}</button>
            <button type="reset" class="btn btn-outline-secondary" data-bs-dismiss="offcanvas">{{ trans('schoolmanagement/stages.close') }}</button>
        </form>
    </div>
</div>

<!-- Edit Stage -->
<div class="offcanvas offcanvas-end" tabindex="-1" id="edit-stage-modal" aria-labelledby="edit-stage-modalLabel">
    <div class="offcanvas-header border-bottom">
        <h5 class="offcanvas-title">{{ trans('schoolmanagement/stages.editStage') }}</h5>
        <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body mx-0 flex-grow-0 pt-0 h-100">
        <form class="pt-0" id="edit-form" action="{{ route('editStage') }}" method="POST">
            @csrf
...
<!-- Delete Stage -->
<div class="modal fade" id="delete-stage-modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel1">{{ trans('schoolmanagement/stages.deleteStage') }}</h5>
            ...
            <div class="modal-body">
                ...
                <form id="delete-form" action="{{ route('deleteStage') }}" method="POST">
                    @csrf
...
<div class="modal fade" id="delete-selected-modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel1">{{ trans('schoolmanagement/stages.deleteSelectedStages') }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                </button>
            </div>
            <hr style="margin: 0.1rem">
            <div class="modal-body">
                {{ trans('schoolmanagement/stages.deleteWarning') }}
                <form id="delete-selected-form" action="{{ route('deleteSelectedStages') }}" method="POST">
                    @csrf
